...
# pixels large.
html_favicon = '_static/logos/specviz2d.ico'
...
# Acronym map shared by scan_directory_for_links and _generate_loader_formats_js
_TITLE_ACRONYMS = {
    'vo': 'VO',
    'api': 'API',
    'url': 'URL',
...
def _rst_filename_to_title(filename):
    """Convert an RST filename to a display title (e.g. '3d_spectrum.rst' -> '3D Spectrum')."""
    name = filename[:-4].replace('_', ' ').title()
    words = name.split()
    for i, word in enumerate(words):
        replacement = _TITLE_ACRONYMS.get(word.lower())
        if replacement:
            words[i] = replacement
    return ' '.join(words)
...
def _generate_conf_settings_js(app):
    """Generate a JS file exposing build-time settings from conf.py."""
