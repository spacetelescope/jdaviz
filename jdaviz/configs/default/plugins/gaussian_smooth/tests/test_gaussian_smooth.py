...
def test_linking_after_spectral_smooth(cubeviz_helper, spectrum1d_cube):
    ...
    data_label = 'test'
    cubeviz_helper.load(spectrum1d_cube, data_label=data_label)
    ...
    assert dc.external_links[1].cids2[0] == dc[0].pixel_component_ids[0]
    ...
    assert dc.external_links[2].cids2[0] == dc[0].pixel_component_ids[1]

    # Link 4:
    # Pixel Axis 2 [x] from cube.pixel_component_ids[2]
    # Pixel Axis 2 [x] from plugin.pixel_component_ids[2]
    assert dc.external_links[3].cids1[0] == dc[-1].pixel_component_ids[2]
    assert dc.external_links[3].cids2[0] == dc[0].pixel_component_ids[2]
...
def test_specviz2d_smooth(specviz2d_helper, spectrum2d):
    ...
    specviz2d_helper.load(spectrum2d, data_label=data_label)
