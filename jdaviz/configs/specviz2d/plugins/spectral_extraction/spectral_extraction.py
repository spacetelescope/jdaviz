...
class SpectralExtraction(PluginTemplateMixin):
    ...
    * :meth:`export_extract_spectrum`
    ...
    def _trace_dataset_selected(self, msg=None):
        ...
        # estimate the pixel number by collapsing along the spectral axis to get a median
        # cross-dispersion profile, then taking the brightest pixel
        trace_flux = self.trace_dataset.get_selected_spectrum(use_display_units=True).flux
        brightest_pixel = int(np.nanmedian(trace_flux, axis=1).argmax())
