...
class TestCatalogs:
    ...
    def test_plugin_image_with_result(self, imviz_helper, tmp_path):
        ...
        assert imviz_helper.viewers['imviz-0']._obj.viewer.state.x_min == 279.0
        assert imviz_helper.viewers['imviz-0']._obj.viewer.state.x_max == 1768.0
        assert imviz_helper.viewers['imviz-0']._obj.viewer.state.y_min == -0.5
        assert imviz_helper.viewers['imviz-0']._obj.viewer.state.y_max == 1488.5
        ...
        assert_allclose(
            imviz_helper.viewers['imviz-0']._obj.viewer.state.x_min, 1046.377555, atol=0.1)
        assert_allclose(
            imviz_helper.viewers['imviz-0']._obj.viewer.state.x_max, 1098.748805, atol=0.1)
        assert_allclose(
            imviz_helper.viewers['imviz-0']._obj.viewer.state.y_min, 699.110485, atol=0.1)
        assert_allclose(
            imviz_helper.viewers['imviz-0']._obj.viewer.state.y_max, 751.481735, atol=0.1)
...
def test_offline_ecsv_catalog(imviz_helper, image_2d_wcs, tmp_path):
    ...
    assert imviz_helper.viewers['imviz-0']._obj.viewer.state.x_min == -0.5
    assert imviz_helper.viewers['imviz-0']._obj.viewer.state.x_max == 9.5
    assert imviz_helper.viewers['imviz-0']._obj.viewer.state.y_min == -0.5
    assert imviz_helper.viewers['imviz-0']._obj.viewer.state.y_max == 9.5
    # Re-populate the table with a new search
    out_tbl = catalogs_plugin.search()
    ...
    assert_allclose(imviz_helper.viewers['imviz-0']._obj.viewer.state.x_min, -0.01966, rtol=1e-4)
    assert_allclose(imviz_helper.viewers['imviz-0']._obj.viewer.state.x_max, 0.02034, rtol=1e-4)
    assert_allclose(imviz_helper.viewers['imviz-0']._obj.viewer.state.y_min, 0.980008, rtol=1e-4)
    assert_allclose(imviz_helper.viewers['imviz-0']._obj.viewer.state.y_max, 1.020008, rtol=1e-4)
...
def test_zoom_to_selected(imviz_helper, image_2d_wcs):
    ...
    xmin, xmax, ymin, ymax = imviz_helper.default_viewer._obj.viewer.get_limits()
...
def test_select_tool(imviz_helper, image_2d_wcs):
    ...
    mark = catalogs_plugin._obj._get_mark(imviz_helper.viewers['imviz-0']._obj.viewer)
