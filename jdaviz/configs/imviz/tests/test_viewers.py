...
def test_zoom_center_radius_init(imviz_helper):
    ...
    arr = np.ones((10, 10))
    imviz_helper.load_data(arr, data_label='my_array')
    assert imviz_helper.default_viewer._obj.viewer.state.zoom_center_x > 0
    assert imviz_helper.default_viewer._obj.viewer.state.zoom_center_y > 0
    assert imviz_helper.default_viewer._obj.viewer.state.zoom_radius > 0
...
def test_catalog_in_image_viewer(imviz_helper, image_2d_wcs, source_catalog):
    ...
    mo._viewer_mouse_event(iv._obj.viewer, {'event': 'mousemove', 'domain': {'x': 0.5, 'y': 0.5}})
