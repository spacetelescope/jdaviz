...
class TestParseImage:
    ...
    def test_parse_hst_drz(self, imviz_helper):
        ...
        assert_quantity_allclose(tbl[0]['sum'], 126.63604794 * data_unit, rtol=1e-3)
        ...
        assert_quantity_allclose(tbl[0]['min'], -0.02858124 * data_unit, rtol=1e-3)
        assert_quantity_allclose(tbl[0]['max'], 3.94278287 * data_unit, rtol=1e-3)
        assert_quantity_allclose(tbl[0]['mean'], 0.04910025 * data_unit, rtol=1e-3)
        assert_quantity_allclose(tbl[0]['median'], 0.02168917 * data_unit, rtol=1e-3)
        ...
        assert_quantity_allclose(tbl[0]['mad_std'], 0.0233135 * data_unit, rtol=1e-3)
        ...
        assert_quantity_allclose(tbl[0]['biweight_midvariance'], 0.00062222 * (data_unit * data_unit), rtol=1e-3)  # noqa
...
@pytest.mark.skipif(RDM_LT_0_14_2, reason="roman_datamodels>=0.14.2.dev required for Roman tests")
def test_roman_wfi(imviz_helper, roman_wfi_image):

    imviz_helper.load_data(roman_wfi_image)

    assert imviz_helper.app.data_collection[0].shape == (20, 20)
