...
def test_asdf_not_rdm(deconfigged_helper):
    ...
    deconfigged_helper.load(af)
    out_component = deconfigged_helper._app.data_collection[0].get_component('DATA')
...
def test_asdf_via_rdm(deconfigged_helper, roman_imagemodel):
    # test support for ASDF files as roman_datamodels ImageModels. In this
    # case, the data don't have units.
    deconfigged_helper.load(roman_imagemodel)
    out_component = deconfigged_helper._app.data_collection[0].get_component('DATA')
