...
class BaseLinkHandler:
    ...
    def check_all_pixel_links(self):
        links = self.imviz._app.data_collection.external_links
        assert len(links) == 2
...
class TestLink_WCS_WCS(BaseImviz_WCS_WCS, BaseLinkHandler):
    ...
    def test_wcslink_affine_with_extras(self):
        ...
        assert 'xy_markers' in self.imviz._app.data_collection.labels
        ...
        ss = self.imviz._app.get_subsets(include_sky_region=True)
        ...
        self.imviz.link_data(align_by='pixels', wcs_fallback_scheme=None)
        assert 'xy_markers' not in self.imviz._app.data_collection.labels
