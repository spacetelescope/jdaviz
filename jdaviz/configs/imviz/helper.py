...
class Imviz(ConfigHelper):
    ...
    def load_static_regions_from_file(self, region_file, region_format='ds9', prefix='region',
        ...
        .. note:: Any unsupported regions will be silently skipped.
        ...
        """
        from regions import Regions, CirclePixelRegion, RectanglePixelRegion, EllipsePixelRegion

        raw_regs = Regions.read(region_file, format=region_format)
        my_regions = {}
        for i, reg in enumerate(raw_regs):
            if isinstance(reg, (CirclePixelRegion, RectanglePixelRegion, EllipsePixelRegion)):
                my_regions[f'{prefix}_{i}'] = reg

        self.load_static_regions(my_regions)
