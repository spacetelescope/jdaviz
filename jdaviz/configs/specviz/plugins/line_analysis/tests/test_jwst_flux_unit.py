...
def _initialize_cubeviz_with_collapse(function, dataset, label):
    ...
    cubeviz_helper.load_spectrum(dataset, data_label=label)
...
def _calculate_line_flux(cubeviz_helper):
    ...
    plugin_index = [ti['name'] for ti in cubeviz_helper.app.state.tray_items].index('specviz-line-analysis')
...
def main_meta_collapse_test(data, metadata, expected_flux_units):
    '''
    Embeds metadata in the main meta, calculates line flux, and checks the units for each collapse function
    '''
...
def primary_header_collapse_test(data, metadata, expected_flux_units):
    '''
    Embeds metadata in the primary header, calculates line flux, and checks the units for each collapse function
    '''
    for function in expected_flux_units:
        cubeviz_helper = _initialize_cubeviz_with_collapse(function, data, cube_label)
        # Manually inject metadata
        for key in metadata:
            cubeviz_helper.app.data_collection[cube_label].meta['_primary_header'][key] = metadata[key]
...
def test_MJy_sr_main_meta(spectrum1d_cube_MJy_sr, test_case):
    ...
    main_meta_collapse_test(spectrum1d_cube_MJy_sr, MJy_sr_test_cases['metadata'], MJy_sr_test_cases['expected_flux_units'])
    # Tests flux calculation units when the metadata is stored in the primary header
    primary_header_collapse_test(spectrum1d_cube_MJy_sr, MJy_sr_test_cases['metadata'], MJy_sr_test_cases['expected_flux_units'])


def test_asdf_metadata(spectrum1d_cube_MJy_sr):
    ''' Test that Line analysis can find the JWST telescope if it's given in the ASDF meta, ignoring the PIXAR_SR value '''
...
def test_Jy(spectrum1d_cube):
    ...
    main_meta_collapse_test(spectrum1d_cube, Jy_test_case)
