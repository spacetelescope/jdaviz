...
def test_prevent_multiple_flux_cubes(deconfigged_helper, spectrum1d_cube):
    ...
    assert cubeviz_helper._app._jdaviz_helper._loaded_flux_cube is not None

    # Store the label of the loaded flux cube
    flux_cube_label = cubeviz_helper._app._jdaviz_helper._loaded_flux_cube.label
    ...
    flux_cube_data = cubeviz_helper._app._jdaviz_helper._loaded_flux_cube

    # Remove it from viewers
    cubeviz_helper._app.remove_data_from_viewer('flux-viewer', flux_cube_label)

    # Remove it from data collection
    cubeviz_helper._app.data_collection.remove(flux_cube_data)
    ...
    all_labels = [d.label for d in cubeviz_helper._app.data_collection]
