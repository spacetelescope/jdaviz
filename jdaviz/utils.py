...
__all__ = ['SnackbarQueue', 'enable_hot_reloading', 'bqplot_clear_figure',
           'standardize_metadata', 'ColorCycler', 'alpha_index', 'alpha_index_to_int',
...
def alpha_index_to_int(label):
    """Inverse of `alpha_index`.

    Parameters
    ----------
    label : str
        A label previously generated by `alpha_index`.

    Returns
    -------
    index : int or None
        The index corresponding to ``label``, or `None` if ``label`` is not a
        valid `alpha_index` output (e.g. an ``mdi-`` icon or a sublayer label).
    """
    if not isinstance(label, str) or not label.isascii() or not label.islower():
        return None
    if len(label) == 1 and 'a' <= label <= 'z':
        return ord(label) - 97
    if len(label) == 2 and all('a' <= char <= 'z' for char in label):
        return (ord(label[0]) - 97 + 1) * 26 + (ord(label[1]) - 97)
    return None
