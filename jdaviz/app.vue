<template>
  <v-app id="web-app" :style="checkNotebookContext() ? 'display: inline' : 'display: flex'" :class="'jdaviz ' + config" ref="mainapp">
    ...
    <v-app-bar color="toolbar" dark :dense="state_settings.dense_toolbar" flat app absolute clipped-right :style="checkNotebookContext() ? 'margin-left: 1px; margin-right: 1px' : ''">

      <v-toolbar-items v-if="config === 'deconfigged'">
        <j-tooltip v-if="(!state_settings.server_is_remote || state_settings.remote_enable_importers)" tipid="app-toolbar-loaders">
          <v-btn icon @click="() => {if (state_drawer_content === 'loaders') {state_drawer_content = ''} else {state_drawer_content = 'loaders'}}" :class="{active : state_drawer_content === 'loaders'}">
            <img :src="state_icons['plus-box']" width="24" class="color-to-white"/>
          </v-btn>
        </j-tooltip>
        <j-tooltip v-if="!state_settings.server_is_remote" tipid="app-toolbar-save">
          <v-btn icon @click="() => {if (state_drawer_content === 'save') {state_drawer_content = ''} else {state_drawer_content = 'save'}}" :class="{active : state_drawer_content === 'save'}" :disabled="!state_tray_items[state_tray_items.map(ti => ti.label).indexOf('Export')].is_relevant">
            <img :src="state_icons['content-save']" width="24" class="color-to-white"/>
          </v-btn>
        </j-tooltip>
        ...
        <j-tooltip tipid="app-toolbar-settings">
          <v-btn icon @click="() => {if (state_drawer_content === 'settings') {state_drawer_content = ''} else {state_drawer_content = 'settings'}}" :class="{active : state_drawer_content === 'settings'}" :disabled="!state_tray_items[state_tray_items.map(ti => ti.label).indexOf('Plot Options')].is_relevant">
            <img :src="state_icons['cog']" width="24" class="color-to-white"/>
          </v-btn>
        </j-tooltip>
        ...
        <j-tooltip tipid="app-toolbar-plugins">
          <v-btn icon @click="() => {if (state_drawer_content === 'plugins') {state_drawer_content = ''} else {state_drawer_content = 'plugins'}}" :class="{active : state_drawer_content === 'plugins'}" :disabled="state_tray_items.filter(ti => {return (ti.is_relevant && ti.sidebar === 'plugins')}).length === 0">
            <img :src="state_icons['tune']" width="24" class="color-to-white"/>
          </v-btn>
        </j-tooltip>
        <j-tooltip tipid="app-toolbar-subsets">
          <v-btn icon @click="() => {if (state_drawer_content === 'subsets') {state_drawer_content = ''} else {state_drawer_content = 'subsets'}}" :class="{active : state_drawer_content === 'subsets'}" :disabled="!state_tray_items[state_tray_items.map(ti => ti.label).indexOf('Plot Options')].is_relevant">
            <img :src="state_subset_mode_create ? state_icons['selection-drag'] : state_icons['selection']" width="24" class="color-to-white"/>
          </v-btn>
        </j-tooltip>
      ...
      <v-toolbar-items v-for="(item, index) in state_tool_items">
        ...
        <j-tooltip v-if="item.name === 'g-data-tools' && config!=='mosviz'" tooltipcontent="Open data menu in sidebar (this button will be removed in a future release)">
          <v-btn tile depressed color="turquoise" @click="state_drawer_content = 'loaders'">
            Import Data
          </v-btn>
        </j-tooltip>
        <j-tooltip v-else-if="config==='mosviz' && item.name == 'g-data-tools' && state_data_items.length !== 0"></j-tooltip>
      ...
      <v-toolbar-items v-if="config !== 'deconfigged'">
        ...
        <j-tooltip v-if="state_show_toolbar_buttons" tipid="app-help">
          <v-btn icon :href="docs_link" target="_blank">
            <v-icon medium>mdi-help-box</v-icon>
          </v-btn>
        </j-tooltip>
        <j-tooltip v-if="state_show_toolbar_buttons && checkNotebookContext()" tipid="app-api-hints">
          <v-btn icon @click="state_show_api_hints = !state_show_api_hints" :class="{active : state_show_api_hints}">
            <img :src="state_icons['api']" width="24" class="invert-if-dark" style="opacity: 1.0"/>
          </v-btn>
        </j-tooltip>
        <v-divider v-if="state_show_toolbar_buttons" vertical style="margin: 0px 10px"></v-divider>
        <j-tooltip v-if="(state_dev_loaders || config!=='mosviz') && (state_show_toolbar_buttons || state_drawer_content === 'loaders') && state_loader_items.length > 0" tipid="app-toolbar-loaders">
          <v-btn icon @click="() => {if (state_drawer_content === 'loaders') {state_drawer_content = ''} else {state_drawer_content = 'loaders'}}" :class="{active : state_drawer_content === 'loaders'}">
            <v-icon medium style="padding-top: 2px">mdi-plus-box</v-icon>
          </v-btn>
        </j-tooltip>
        <j-tooltip v-if="state_show_toolbar_buttons || state_drawer_content === 'logger'" tipid="app-toolbar-logger-configged">
          <v-btn icon @click="() => {if (state_drawer_content === 'logger') {state_drawer_content = ''} else {state_drawer_content = 'logger'}}" :class="{active : state_drawer_content === 'logger'}">
            <v-icon medium style="padding-top: 2px">mdi-message-reply-text</v-icon>
          </v-btn>
        </j-tooltip>
        <j-tooltip v-if="state_show_toolbar_buttons || state_drawer_content === 'plugins'" tipid="app-toolbar-plugins-configged">
          <v-btn icon @click="() => {if (state_drawer_content === 'plugins') {state_drawer_content = ''} else {state_drawer_content = 'plugins'}}" :class="{active : state_drawer_content === 'plugins'}">
      ...
      <v-toolbar-items v-if="config === 'deconfigged'">
        <v-layout column  class="app-bar-right" style="height: 28px; padding-bottom: 12px; margin-top: 2px" v-if="state_show_toolbar_buttons || state_global_search_menu || state_about_popup">
              ...
              <v-card style="min-width: 350px; max-height: 500px; overflow-y: scroll">
                <v-container>
                  <div v-for="ldrItem in loader_items_filtered" :key="ldrItem.label">
                    <v-row v-if="trayItemVisible(ldrItem, state_global_search)">
                      <v-list-item style="display: grid; min-height: 6px; cursor: pointer" @click="(e) => {search_item_clicked({attr: 'loaders', label: ldrItem.label})}">
                        ...
                        <v-list-item-subtitle v-if="state_show_api_hints && state_global_search.length" v-for="api_method in trayItemMethodMatch(ldrItem, state_global_search)" style="white-space: normal; font-size: 8pt; padding-top: 4px; padding-bottom: 4px" class="api-hint">
                  ...
                  <div v-for="vcItem in state_new_viewer_items" :key="vcItem.label">
                    <v-row v-if="vcItem.is_relevant && trayItemVisible(vcItem, state_global_search)">
                      <v-list-item style="display: grid; min-height: 6px; cursor: pointer" @click="(e) => {search_item_clicked({attr: 'new_viewers', label: vcItem.label})}">
                        ...
                        <v-list-item-subtitle v-if="state_show_api_hints && state_global_search.length" v-for="api_method in trayItemMethodMatch(vcItem, state_global_search)" style="white-space: normal; font-size: 8pt; padding-top: 4px; padding-bottom: 4px" class="api-hint">
                  ...
                  <div v-for="dmItem in state_viewer_items" :key="dmItem.name">
                    <v-row v-if="trayItemVisible(dmItem, state_global_search)">
                      <v-list-item style="display: grid; min-height: 6px; cursor: pointer" @click="(e) => {search_item_clicked({attr: 'data_menus', label: dmItem.name})}">
                        ...
                        <v-list-item-subtitle v-if="state_show_api_hints && state_global_search.length" v-for="api_method in trayItemMethodMatch(dmItem, state_global_search)" style="white-space: normal; font-size: 8pt; padding-top: 4px; padding-bottom: 4px" class="api-hint">
                  ...
                  <div v-for="(trayItem, index) in state_tray_items" :key="index">
                    <v-row v-if="trayItem.is_relevant && trayItemVisible(trayItem, state_global_search)">
                      <v-list-item style="display: grid; min-height: 6px; cursor: pointer" @click="(e) => {search_item_clicked({attr: 'plugins', label: trayItem.label})}">
                        ...
                        <v-list-item-subtitle v-if="state_show_api_hints && state_global_search.length" v-for="api_method in trayItemMethodMatch(trayItem, state_global_search)" style="white-space: normal; font-size: 8pt; padding-top: 4px; padding-bottom: 4px" class="api-hint">
          ...
          <span style="display: inline-flex; align-items: center; margin-top: 4px;">
            ...
            <j-about-menu
              :jdaviz_version="state_jdaviz_version"
              :api_hints_obj="api_hints_obj"
              :api_hints_enabled="state_show_api_hints"
              :about_widget="state_tray_items[state_tray_items.map(ti => ti.label).indexOf('About')].widget"
              :force_open_about.sync="force_open_about"
            ></j-about-menu>

            <j-tooltip v-if="state_show_toolbar_buttons && checkNotebookContext()" tipid="app-api-hints">
              <v-btn small icon @click="state_show_api_hints = !state_show_api_hints" :class="{active : state_show_api_hints}">
                <img :src="state_icons['api']" width="24" class="color-to-white" style="opacity: 1.0; padding-top: 2px; padding-bottom: 2px"/>
              </v-btn>
            </j-tooltip>
            <j-tooltip tipid="app-toolbar-popout" span_style="scale: 0.8; margin-left: -4px; margin-right: -4px">
              <jupyter-widget :widget="popout_button" ></jupyter-widget>
    ...
    <v-content
      :style="checkNotebookContext() ? 'height: ' + state_settings.context.notebook.max_height + '; border: solid 1px #e5e5e5; border-top: 0px' : ''"
      :class="checkNotebookContext() ? '' : 'jdaviz__content--not-in-notebook'"
    >
      <v-container class="fill-height pa-0" fluid>
        <splitpanes>
          <pane size="25" min-size="25" v-if="config === 'deconfigged' && state_drawer_content.length > 0" style="background-color: #fafbfc; border-top: 6px solid #C75109; min-width: 320px">
            <v-card v-if="state_drawer_content === 'loaders'" flat tile class="fill-height" style="overflow-x: hidden; overflow-y: hidden" color="gray">
              <v-tabs fixed-tabs dark background-color="viewer_toolbar" v-model="state_add_subtab">
                <v-tab>Data</v-tab>
                <v-tab>Viewer</v-tab>
              </v-tabs>
              <v-tabs-items v-model="state_add_subtab" style="overflow-y: auto">
                <v-tab-item style="padding-bottom: 40px">
                  <j-loader-panel
                    :loader_items="state_loader_items"
                    :loader_selected.sync="state_loader_selected"
                    :api_hints_enabled="state_show_api_hints"
                    :api_hints_obj="api_hints_obj || config"
                    :server_is_remote="state_settings.server_is_remote"
                    :disabled_loaders="state_settings.disabled_loaders"
                  ></j-loader-panel>
                </v-tab-item>
                <v-tab-item style="padding-bottom: 40px">
                  <j-new-viewer-panel
                    :new_viewer_items="state_new_viewer_items"
                    :new_viewer_selected.sync="state_new_viewer_selected"
                    :api_hints_enabled="state_show_api_hints"
                    :api_hints_obj="api_hints_obj || config"
                  ></j-new-viewer-panel>
                </v-tab-item>
            ...
            <v-card v-if="state_drawer_content === 'save' && !state_settings.server_is_remote" flat tile class="overflow-y-auto fill-height" style="overflow-x: hidden" color="gray">
              ...
              <jupyter-widget :widget="state_tray_items[state_tray_items.map(ti => ti.label).indexOf('Export')].widget"></jupyter-widget>
            </v-card>
            <v-card v-if="state_drawer_content === 'plugins'" flat tile class="overflow-y-auto fill-height" style="overflow-x: hidden" color="gray">
              <v-text-field
                ...
                hide-details
              ></v-text-field>
              <v-expansion-panels accordion multiple focusable flat tile v-model="state_tray_items_open">
                <v-expansion-panel v-for="(trayItem, index) in state_tray_items" :key="index">
                  <div v-if="trayItem.is_relevant && trayItemVisible(trayItem, state_tray_items_filter) && (trayItem.sidebar === 'plugins' || config !== 'deconfigged')">
                    <v-expansion-panel-header class="plugin-header">
                      <v-list-item style="display: grid; min-height: 6px" class="plugin-title">
                        ...
                        <v-list-item-subtitle v-if="state_show_api_hints && state_tray_items_filter.length" v-for="api_method in trayItemMethodMatch(trayItem, state_tray_items_filter)" style="white-space: normal; font-size: 8pt; padding-top: 4px; padding-bottom: 4px" class="api-hint">
                          <span class="api-hint">plg.{{ api_method }}</span>
                        </v-list-item-subtitle>
                        <v-list-item-subtitle style="white-space: normal; font-size: 8pt">
                          {{ trayItem.tray_item_description }}
                    ...
                    <v-expansion-panel-content style="margin-left: -12px; margin-right: -12px;">
                      <jupyter-widget v-if="state_tray_items_open.includes(index)" :widget="trayItem.widget"></jupyter-widget>
            ...
            <v-card v-if="state_drawer_content === 'info'" flat tile class="fill-height" style="overflow-x: hidden; overflow-y: hidden" color="gray">
              <v-tabs fixed-tabs dark background-color="viewer_toolbar" v-model="state_info_subtab">
                <v-tab>Metadata</v-tab>
                <v-tab>Markers</v-tab>
                <v-tab>Logger</v-tab>
              </v-tabs>
              <v-tabs-items v-model="state_info_subtab" style="overflow-y: auto">
                <v-tab-item style="padding-bottom: 40px">
                  <span v-if="state_show_api_hints" class="api-hint" style="font-weight: bold">plg = {{  api_hints_obj || config }}.plugins['Metadata']</span>
                  <jupyter-widget :widget="state_tray_items[state_tray_items.map(ti => ti.label).indexOf('Metadata')].widget"></jupyter-widget>
                </v-tab-item>
                <v-tab-item style="padding-bottom: 40px">
                  <span v-if="state_show_api_hints" class="api-hint" style="font-weight: bold">plg = {{  api_hints_obj || config }}.plugins['Markers']</span>
                  <jupyter-widget :widget="state_tray_items[state_tray_items.map(ti => ti.label).indexOf('Markers')].widget"></jupyter-widget>
                </v-tab-item>
                <v-tab-item style="padding-bottom: 40px">
                  <span v-if="state_show_api_hints" class="api-hint" style="font-weight: bold">plg = {{  api_hints_obj || config }}.plugins['Logger']</span>
                  <jupyter-widget :widget="state_tray_items[state_tray_items.map(ti => ti.label).indexOf('Logger')].widget"></jupyter-widget>
                </v-tab-item>
              </v-tabs-items>
            </v-card>
            <v-card v-if="state_drawer_content === 'subsets'" flat tile class="overflow-y-auto fill-height" style="overflow-x: hidden" color="gray">
              ...
              <jupyter-widget :widget="state_tray_items[state_tray_items.map(ti => ti.label).indexOf('Subset Tools')].widget"></jupyter-widget>
            </v-card>
            <v-card v-if="state_drawer_content === 'settings'" flat tile class="fill-height" style="overflow-x: hidden; overflow-y: hidden" color="gray">
              <v-tabs fixed-tabs dark background-color="viewer_toolbar" v-model="state_settings_subtab">
                <v-tab :disabled="!state_tray_items[state_tray_items.map(ti => ti.label).indexOf('Plot Options')].is_relevant">Plot Options</v-tab>
                <v-tab>Units</v-tab>
              </v-tabs>
              <v-tabs-items v-model="state_settings_subtab" style="overflow-y: auto">
                <v-tab-item style="padding-bottom: 40px">
                  <span v-if="state_show_api_hints" class="api-hint" style="font-weight: bold">plg = {{  api_hints_obj || config }}.plugins['Plot Options']</span>
                  <jupyter-widget :widget="state_tray_items[state_tray_items.map(ti => ti.label).indexOf('Plot Options')].widget"></jupyter-widget>
                </v-tab-item>
                <v-tab-item style="padding-bottom: 40px">
                  ...
                  <jupyter-widget :widget="state_tray_items[state_tray_items.map(ti => ti.label).indexOf('Unit Conversion')].widget"></jupyter-widget>
                </v-tab-item>
              </v-tabs-items>
            </v-card>
          ...
          <pane size="25" min-size="25" v-if="config !== 'deconfigged' && state_drawer_content.length > 0" style="background-color: #fafbfc; border-top: 6px solid #C75109; min-width: 250px">
            <v-card v-if="state_drawer_content === 'loaders'" flat tile class="overflow-y-auto fill-height" style="overflow-x: hidden" color="gray">
              <j-loader-panel
                :loader_items="state_loader_items"
                :loader_selected.sync="state_loader_selected"
                :api_hints_enabled="state_show_api_hints"
            ...
            <v-card v-if="state_drawer_content === 'logger'" flat tile class="overflow-y-auto fill-height" style="overflow-x: hidden" color="gray">
              <jupyter-widget :widget="state_tray_items[state_tray_items.map(ti => ti.label).indexOf('Logger')].widget"></jupyter-widget>
            </v-card>
            ...
            <v-card v-if="state_drawer_content === 'plugins'" flat tile class="overflow-y-auto fill-height" style="overflow-x: hidden" color="gray">
              ...
              <v-expansion-panels accordion multiple focusable flat tile v-model="state_tray_items_open">
                <v-expansion-panel v-for="(trayItem, index) in state_tray_items" :key="index">
                  <div v-if="trayItem.is_relevant && trayItemVisible(trayItem, state_tray_items_filter) && trayItem.label !== 'Logger'">
                    <v-expansion-panel-header class="plugin-header">
                      <v-list-item style="display: grid; min-height: 6px" class="plugin-title">
                        <v-list-item-title>
                          <j-tooltip :tipid="trayItem.name">
    ...
    <v-snackbar
      v-model="state_snackbar.show"
      :timeout="state_snackbar.timeout"
      :color="state_snackbar.color"
      top
      right
...
export default {
  ...
  computed: {
    loader_items_filtered() {
      ...
      return this.state_loader_items.filter(item => {
