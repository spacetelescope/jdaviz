<template>
  <v-app
    id="web-app"
    :style="checkNotebookContext() ? 'display: inline; --jdaviz-notebook-max-height: ' + settings.context.notebook.max_height : 'display: flex'"
  ...
  >
    ...
    <v-app-bar color="toolbar" theme="dark" :density="settings.dense_toolbar ? 'compact' : 'default'" flat absolute :style="checkNotebookContext() ? 'margin-left: 1px; margin-right: 1px' : ''">
      ...
      <v-toolbar-items v-for="(item, index) in tool_items" :key="item.name || index">
        ...
        <j-tooltip v-if="item.name === 'g-data-tools' && config!=='mosviz'" tooltipcontent="Open data menu in sidebar (this button will be removed in a future release)">
          <v-btn rounded="0" variant="flat" color="turquoise" @click="drawer_content = 'loaders'">
            Import Data
          </v-btn>
        </j-tooltip>
      ...
      <v-toolbar-items v-if="config !== 'deconfigged'">
        <j-tooltip tipid="app-toolbar-popout" span_style="scale: 0.8; margin-left: -4px; margin-right: -4px">
          <jupyter-widget v-if="!in_notebook || checkNotebookContext()" :widget="popout_button" :key="popout_button"></jupyter-widget>
        </j-tooltip>
        ...
        <j-tooltip v-if="show_toolbar_buttons && (checkNotebookContext() || in_notebook)" tipid="app-api-hints">
      ...
      <v-toolbar-items v-if="config === 'deconfigged'">
        <div class="app-bar-right d-flex flex-column" style="height: 28px; padding-bottom: 12px; margin-top: 2px" v-if="show_toolbar_buttons || global_search_menu || about_popup">
              ...
              <v-card style="min-width: 350px; max-height: 500px; overflow-y: scroll">
                <v-container>
                  <div v-for="ldrItem in loader_items_filtered" :key="ldrItem.label">
                    <j-flex-row v-if="trayItemVisible(ldrItem, global_search)">
                      <v-list-item style="display: grid; min-height: 6px; cursor: pointer" @click="(e) => {search_item_clicked({attr: 'loaders', label: ldrItem.label})}">
                        ...
                        <template v-if="show_api_hints && global_search.length">
                          <v-list-item-subtitle v-for="api_method in trayItemMethodMatch(ldrItem, global_search)" :key="`loader-${ldrItem.label}-${api_method}`" style="white-space: normal; font-size: 8pt; padding-top: 4px; padding-bottom: 4px" class="api-hint">
                  ...
                  <div v-for="vcItem in new_viewer_items" :key="vcItem.label">
                    <j-flex-row v-if="vcItem.is_relevant && trayItemVisible(vcItem, global_search)">
                      <v-list-item style="display: grid; min-height: 6px; cursor: pointer" @click="(e) => {search_item_clicked({attr: 'new_viewers', label: vcItem.label})}">
                        ...
                        <template v-if="show_api_hints && global_search.length">
                          <v-list-item-subtitle v-for="api_method in trayItemMethodMatch(vcItem, global_search)" :key="`new-viewer-${vcItem.label}-${api_method}`" style="white-space: normal; font-size: 8pt; padding-top: 4px; padding-bottom: 4px" class="api-hint">
                  ...
                  <div v-for="dmItem in viewer_items" :key="dmItem.name">
                    <j-flex-row v-if="trayItemVisible(dmItem, global_search)">
                      <v-list-item style="display: grid; min-height: 6px; cursor: pointer" @click="(e) => {search_item_clicked({attr: 'data_menus', label: dmItem.name})}">
                        ...
                        <template v-if="show_api_hints && global_search.length">
                          <v-list-item-subtitle v-for="api_method in trayItemMethodMatch(dmItem, global_search)" :key="`data-menu-${dmItem.name}-${api_method}`" style="white-space: normal; font-size: 8pt; padding-top: 4px; padding-bottom: 4px" class="api-hint">
                  ...
                  <div v-for="(trayItem, index) in tray_items" :key="index">
                    <j-flex-row v-if="trayItem.is_relevant && trayItemVisible(trayItem, global_search)">
                      <v-list-item style="display: grid; min-height: 6px; cursor: pointer" @click="(e) => {search_item_clicked({attr: 'plugins', label: trayItem.label})}">
                        ...
                        <template v-if="show_api_hints && global_search.length">
                          <v-list-item-subtitle v-for="api_method in trayItemMethodMatch(trayItem, global_search)" :key="`tray-${trayItem.name}-${api_method}`" style="white-space: normal; font-size: 8pt; padding-top: 4px; padding-bottom: 4px" class="api-hint">
        ...
        <splitpanes>
          <pane size="25" min-size="25" v-if="config === 'deconfigged' && drawer_content.length > 0" style="background-color: #fafbfc; border-top: 6px solid #C75109; min-width: 320px">
            <v-card v-if="drawer_content === 'loaders'" flat tile class="fill-height" style="overflow-x: hidden; overflow-y: hidden" color="gray">
              <v-tabs fixed-tabs theme="dark" bg-color="viewer_toolbar" v-model="add_subtab">
                <v-tab>Data</v-tab>
                <v-tab>Viewer</v-tab>
              </v-tabs>
              <v-window v-model="add_subtab" style="overflow-y: auto">
                ...
                <v-window-item style="padding-bottom: 40px">
                  <j-new-viewer-panel
                    ...
                    v-model:new_viewer_selected="new_viewer_selected"
                    :api_hints_enabled="show_api_hints"
                    :api_hints_obj="api_hints_obj || config"
                  ></j-new-viewer-panel>
                </v-window-item>
            ...
            <v-card v-if="drawer_content === 'plugins'" flat tile class="overflow-y-auto fill-height" style="overflow-x: hidden" color="gray">
              ...
              <v-expansion-panels accordion multiple focusable flat tile v-model="tray_items_open">
                <v-expansion-panel v-for="(trayItem, index) in tray_items" :key="index">
                  <template v-if="trayItem.is_relevant && trayItemVisible(trayItem, tray_items_filter) && (trayItem.sidebar === 'plugins' || config !== 'deconfigged')">
                    <v-expansion-panel-title class="plugin-header">
                      <div class="d-flex flex-column pl-4">
                        ...
                        <div v-if="show_api_hints" style="white-space: normal; font-size: 8pt; padding-top: 4px; padding-bottom: 4px" class="api-hint">
                        ...
                        <template v-if="show_api_hints && tray_items_filter.length">
                          <div v-for="api_method in trayItemMethodMatch(trayItem, tray_items_filter)" :key="`drawer-decfg-${trayItem.name}-${api_method}`" style="white-space: normal; font-size: 8pt; padding-top: 4px; padding-bottom: 4px" class="api-hint">
                            <span class="api-hint">plg.{{ api_method }}</span>
                          </div>
                        </template>
                    ...
                    <v-expansion-panel-text style="margin-left: -12px; margin-right: -12px;">
                      <jupyter-widget v-if="tray_items_open.includes(index) && trayItem.widget" :widget="trayItem.widget" :key="trayItem.widget"></jupyter-widget>
            ...
            <v-card v-if="drawer_content === 'info'" flat tile class="fill-height" style="overflow-x: hidden; overflow-y: hidden" color="gray">
              <v-tabs fixed-tabs theme="dark" bg-color="viewer_toolbar" v-model="info_subtab">
                <v-tab>Metadata</v-tab>
                <v-tab>Markers</v-tab>
                <v-tab>Logger</v-tab>
              </v-tabs>
              <v-window v-model="info_subtab" style="overflow-y: auto">
                <v-window-item style="padding-bottom: 40px">
            ...
            <v-card v-if="drawer_content === 'settings'" flat tile class="fill-height" style="overflow-x: hidden; overflow-y: hidden" color="gray">
              <v-tabs fixed-tabs theme="dark" bg-color="viewer_toolbar" v-model="settings_subtab">
                <v-tab :disabled="!tray_items[tray_items.map(ti => ti.label).indexOf('Plot Options')].is_relevant">Plot Options</v-tab>
                <v-tab>Units</v-tab>
              </v-tabs>
              <v-window v-model="settings_subtab" style="overflow-y: auto">
                <v-window-item style="padding-bottom: 40px">
          ...
          <pane size="25" min-size="25" v-if="config !== 'deconfigged' && drawer_content.length > 0" style="background-color: #fafbfc; border-top: 6px solid #C75109; min-width: 250px">
            ...
            <v-card v-if="drawer_content === 'plugins'" flat tile class="overflow-y-auto fill-height" style="overflow-x: hidden" color="gray">
              ...
              <v-expansion-panels accordion multiple focusable flat tile v-model="tray_items_open">
                <v-expansion-panel v-for="(trayItem, index) in tray_items" :key="index">
                  <template v-if="trayItem.is_relevant && trayItemVisible(trayItem, tray_items_filter) && trayItem.label !== 'Logger'">
                    <v-expansion-panel-title class="plugin-header">
                      <div class="d-flex flex-column pl-4">
                        <div class="text-subtitle-1">
                          <j-tooltip :tipid="trayItem.name">
                        ...
                        <template v-if="show_api_hints && tray_items_filter.length">
                          <div v-for="api_method in trayItemMethodMatch(trayItem, tray_items_filter)" :key="`drawer-cfg-${trayItem.name}-${api_method}`" style="white-space: normal; font-size: 8pt; padding-top: 4px; padding-bottom: 4px" class="api-hint">
                            <span class="api-hint">plg.{{ api_method }}</span>
                          </div>
                        </template>
                        <div class="text-caption text-medium-emphasis">
