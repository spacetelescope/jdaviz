<template>
  ...
  >
    ...
    <v-app-bar color="toolbar" theme="dark" :density="state_settings.dense_toolbar ? 'compact' : 'default'" flat absolute :style="checkNotebookContext() ? 'margin-left: 1px; margin-right: 1px' : ''">
      ...
      <v-toolbar-items v-for="(item, index) in state_tool_items" :key="item.name || index">
        ...
        <j-tooltip v-if="item.name === 'g-data-tools' && config!=='mosviz'" tooltipcontent="Open data menu in sidebar (this button will be removed in a future release)">
          <v-btn rounded="0" variant="flat" color="turquoise" @click="state_drawer_content = 'loaders'">
            Import Data
          </v-btn>
        </j-tooltip>
      ...
      <v-toolbar-items v-if="config !== 'deconfigged'">
        <j-tooltip tipid="app-toolbar-popout" span_style="scale: 0.8; margin-left: -4px; margin-right: -4px">
          <jupyter-widget v-if="!state_in_notebook || checkNotebookContext()" :widget="popout_button" :key="popout_button"></jupyter-widget>
        </j-tooltip>
        ...
        <j-tooltip v-if="state_show_toolbar_buttons && (checkNotebookContext() || state_in_notebook)" tipid="app-api-hints">
      ...
      <v-toolbar-items v-if="config === 'deconfigged'">
        <div class="app-bar-right d-flex flex-column" style="height: 28px; padding-bottom: 12px; margin-top: 2px" v-if="state_show_toolbar_buttons || state_global_search_menu || state_about_popup">
          ...
          <span style="display: inline-flex; align-items: center; margin-top: 4px;">
            ...
            <j-about-menu
              ...
              :downstream_packages="state_downstream_packages"
        ...
        <splitpanes>
          <pane size="25" min-size="25" v-if="config === 'deconfigged' && state_drawer_content.length > 0" style="background-color: #fafbfc; border-top: 6px solid #C75109; min-width: 320px">
            <v-card v-if="state_drawer_content === 'loaders'" flat tile class="fill-height" style="overflow-x: hidden; overflow-y: hidden" color="gray">
              <v-tabs fixed-tabs theme="dark" bg-color="viewer_toolbar" v-model="state_add_subtab">
                <v-tab>Data</v-tab>
                <v-tab>Viewer</v-tab>
              </v-tabs>
              <v-window v-model="state_add_subtab" style="overflow-y: auto">
                <v-window-item style="padding-bottom: 40px">
                  <j-loader-panel
                    :loader_items="state_loader_items"
                    v-model:loader_selected="state_loader_selected"
                    :api_hints_enabled="state_show_api_hints"
                    ...
                    :in_notebook="state_in_notebook"
                  ></j-loader-panel>
                </v-window-item>
                <v-window-item style="padding-bottom: 40px">
                  <j-new-viewer-panel
                    :new_viewer_items="state_new_viewer_items"
                    v-model:new_viewer_selected="state_new_viewer_selected"
                    :api_hints_enabled="state_show_api_hints"
                    :api_hints_obj="api_hints_obj || config"
                  ></j-new-viewer-panel>
                </v-window-item>
            ...
            <v-card v-if="state_drawer_content === 'plugins'" flat tile class="overflow-y-auto fill-height" style="overflow-x: hidden" color="gray">
              ...
              <v-expansion-panels accordion multiple focusable flat tile v-model="state_tray_items_open">
                <v-expansion-panel v-for="(trayItem, index) in state_tray_items" :key="index">
                  <template v-if="trayItem.is_relevant && trayItemVisible(trayItem, state_tray_items_filter) && (trayItem.sidebar === 'plugins' || config !== 'deconfigged')">
                    ...
                    <v-expansion-panel-text style="margin-left: -12px; margin-right: -12px;">
                      <jupyter-widget v-if="state_tray_items_open.includes(index) && trayItem.widget" :widget="trayItem.widget" :key="trayItem.widget"></jupyter-widget>
            ...
            <v-card v-if="state_drawer_content === 'settings'" flat tile class="fill-height" style="overflow-x: hidden; overflow-y: hidden" color="gray">
              <v-tabs fixed-tabs theme="dark" bg-color="viewer_toolbar" v-model="state_settings_subtab">
          ...
          <pane size="25" min-size="25" v-if="config !== 'deconfigged' && state_drawer_content.length > 0" style="background-color: #fafbfc; border-top: 6px solid #C75109; min-width: 250px">
            ...
            <v-card v-if="state_drawer_content === 'plugins'" flat tile class="overflow-y-auto fill-height" style="overflow-x: hidden" color="gray">
              ...
              <v-expansion-panels accordion multiple focusable flat tile v-model="state_tray_items_open">
                <v-expansion-panel v-for="(trayItem, index) in state_tray_items" :key="index">
                  <template v-if="trayItem.is_relevant && trayItemVisible(trayItem, state_tray_items_filter) && trayItem.label !== 'Logger'">
                    <v-expansion-panel-title class="plugin-header">
                      <div class="d-flex flex-column pl-4">
                        <div class="text-subtitle-1">
                          <j-tooltip :tipid="trayItem.name">
...
export default {
  ...
  computed: {
    ...
    snackbarTimeout() {
      ...
      return Number(this.state_snackbar.timeout) === 0 ? -1 : this.state_snackbar.timeout;
    },
  },
  methods: {
    ...
    trayWidget(label) {
      ...
      return this.state_tray_items[index].widget || '';
    }
  },
  mounted() {
