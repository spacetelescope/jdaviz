...
def test_mos_importer_is_valid(deconfigged_helper, tmp_path):
    ...
    (compressed_dir / 'jw00002_x1d.fits.gz').touch()
    importer._input = str(compressed_dir)
    assert importer._check_is_valid() == ''

    hidden_dir = tmp_path / 'hidden'
    hidden_dir.mkdir()
    (hidden_dir / '.jw00003_x1d.fits').touch()
    ...
    (unsupported_file_dir / 'jw00004_x1d.fits').touch()
    ...
    (spectrum_2d_and_image_dir / 'jw00005_s2d.fits').touch()
    (spectrum_2d_and_image_dir / 'jw00005_i2d.fits').touch()
