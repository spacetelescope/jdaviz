...
def test_catalog_visibility(imviz_helper, image_2d_wcs):
    ...
    # uncomment once visibility toggling when changing link type
    # when reference data doesnt change is fixed. for now just verify that the
    # last loaded catalog is visible
    # assert dm.data_labels_visible == ['catalog2', 'Image[DATA]']
    assert 'catalog2' in dm.data_labels_visible
