...
class SinglePixelRegion(CheckableTool):
    ...
    def on_mouse_event(self, data):
        ...
        if data['altKey'] is True:
            ...
            self.viewer.jdaviz_app.get_tray_item_from_name('g-subset-plugin').combination_mode = 'new'  # noqa
            self.viewer.jdaviz_app.get_tray_item_from_name('g-subset-plugin').import_region(
                reg, return_bad_regions=True)
            self.viewer.jdaviz_app.get_tray_item_from_name('g-subset-plugin').combination_mode = 'replace'  # noqa
