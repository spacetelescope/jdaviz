...
class AIDAMixin:
    ...
    def _set_fov(self, fov, image_label=None):
        if fov is None:
            return

        if isinstance(fov, (u.Quantity, Angle)):
            current_fov = self._get_current_fov('sky', image_label=image_label)
        elif isinstance(fov, (float, int)):
            current_fov = self._get_current_fov('pixel', image_label=image_label)
        else:
            raise ValueError(
    ...
    def set_viewport(self, center=None, fov=None, rotation=None, image_label=None, **kwargs):
        ...
        self._set_fov(fov, image_label=image_label)
    ...
    def _get_reference_data(self, image_label=None):
        orientation = self.jdaviz_app._jdaviz_helper.plugins.get('Orientation', None)
    ...
    def _get_current_fov(self, sky_or_pixel=None, image_label=None):
        state = self.state
        reference_data = self._get_reference_data(image_label=image_label)
        wcs = reference_data.coords
    ...
    def _get_current_rotation(self, image_label=None):
    ...
    def _get_current_projection(self, image_label=None):
        """
        ...
        if reference_data.coords is None or not hasattr(reference_data.coords, 'wcs'):
            return None

        return reference_data.coords.wcs.ctype[0][5:8]
    ...
    def get_viewport(self, sky_or_pixel=None, image_label=None, **kwargs):
        ...
        return dict(
            ...
            rotation=self._get_current_rotation(image_label=image_label),
            projection=self._get_current_projection(image_label=image_label),
