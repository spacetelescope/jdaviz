...
class JdavizTableWidget(TableGlue):
    ...
    All business logic (traitlets + vue_* handlers) lives here in jdaviz so
    that glue-jupyter only needs the minimal ``_widget_table_cls`` hook.
    """
    ...
    # Python → Vue: set to a column name to make Vue enter rename mode for
    # that header immediately (used after programmatically adding a column).
