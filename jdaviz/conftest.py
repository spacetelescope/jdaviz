...
def _enabled():
    """
    Return whether memory profiling is enabled via environment.
    """
    return os.getenv('MEMORY_PROFILE') == '1'


def pytest_sessionstart(session):
    """
    Start tracemalloc at test session start if enabled.
    """
    if not _enabled():
        return
    tracemalloc.start(25)  # record 25 frames
...
def pytest_runtest_setup(item):
    ...
    item._mem_rss_start = rss
    item._mem_snapshot_start = tracemalloc.take_snapshot()
    start_t = time.time()
...
def pytest_runtest_teardown(item):
    """
    Snapshot RSS and tracemalloc after each test finishes and print a
    short report (delta RSS and top tracemalloc differences).
    """
    if not _enabled():
        return
    proc = psutil.Process()
    rss_end = proc.memory_info().rss
    rss_start = getattr(item, '_mem_rss_start', None)
    snap_start = getattr(item, '_mem_snapshot_start', None)
    duration = time.time() - getattr(item, '_mem_time_start', 0.0)

    if rss_start is not None:
        delta_mb = (rss_end - rss_start) / 1024**2
        print(f'ENDED_MEM: {item.nodeid} end_rss={rss_end/1024**2:.2f}MiB'
              f' delta={delta_mb:+.2f}MiB dur={duration:.2f}s',
              flush=True)
    else:
        print(f'ENDED_MEM: {item.nodeid} end_rss={rss_end/1024**2:.2f}MiB',
              flush=True)
    ...
    if snap_start is not None:
        ...
        stats = snap_end.compare_to(snap_start, 'lineno')[:10]
        print('TRACEMALLOC_TOP: (top 10 diffs)', flush=True)
        for stat in stats:
            size_kb = stat.size_diff / 1024
            count = stat.count_diff
            frame = stat.traceback[0]
            print(f'  {size_kb:+.1f} KiB {count:+d} '
